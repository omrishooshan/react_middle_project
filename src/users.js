  ...
  const classes = useStyles();
   const[typedLetter,getLetter]=useState("")
    ...
    const addNewUser=()=>{
      ...
      let obj={}; obj.id=maxValue+1; obj.name = name; obj.email=email; obj.city =""; obj.zipcode =""; obj.street ="";
    ...
    useEffect((props)=>{
        ...
        utils.getUsers().then(x=>{getusers(x)
    ...
    // }
        ...
        return(
            ...
            <AppBar>
                <Toolbar>        <h3>Welcome To Users Programme</h3>
                <SearchIcon className={classes.srcicon}/> 
             <TextField className={classes.srch} 
             id="outlined-basic" label="Search User" value={typedLetter}
              onChange={e=>getLetter(e.target.value)} variant="outlined" color="secondary" />
               <Button className={classes.add}  variant="contained" onClick={()=>changepopup(true)}  color="secondary" > Add User</Button> 
                          ...
                          <div className={ !ispopupopen ? classes.popupHide :classes.popup }>
                                  ...
                                  <TextField className={classes.newUS} onChange={e=>getNewName(e.target.value)} label="Name:" color="primary" />
                                  <TextField className={classes.newUS} onChange={e=>getNewMail(e.target.value)} label="Email:" color="primary" />
                                    </div>
                                    <div className={classes.footer}>
                                    <Button  variant="contained"  color="secondary" onClick={addNewUser} > Add </Button> 
