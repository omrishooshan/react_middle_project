...
const addNewTodo=()=>{
  ...
  let obj={}; obj.id=maxValue+1; obj.title = title;  obj.userId =tasksid; obj.completed= false
   ...
   changepopup(false)
            ...
            <div className={classes.root}>
                  ...
                  <div className={ !ispopupopen ? classes.popupHide :classes.popup }>
                                  ...
                                  <TextField className={classes.newpos} onChange={e=>getNewTitle(e.target.value)} label="title:" color="primary" />
                                    ...
                                    <Button  variant="contained"  color="secondary" onClick={addNewTodo} > Add </Button> 
                                    <Button  variant="contained" onClick={()=>changepopup(false)}  color="secondary" > Cancel</Button> 
                                    </div>
                          </div>
                  <Button onClick={()=>changepopup(true)}  size="small" className={classes.posttodobutton}  variant="contained"  color="secondary"> Add Todo</Button>
        ...
        <div className={classes.t}>TASKS</div>
                    ...
                    <Grid container spacing={6} className={classes.gg} >
                               <div className="right"> 
                    {  
                      tasks.filter(x=>x.userId == tasksid).map((x,index)=>{
                           
                        return     <Paper key={index} elevation={3} className="tasks" > 
                                 <Task key={index} mrkComplete={(specificTaskId)=>markCompleted(specificTaskId)} mytasks={x}/>
                                 </Paper>
